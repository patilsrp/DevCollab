...
export function createSecureRoomId(): string {
  return generateRoomId();
}

// Generate a user-friendly room code (shorter, easier to share)
export function createFriendlyRoomCode(): string {
...
export function createTimedRoomId(prefix: string = 'room'): string {
...
// Room ID generator interface
export interface IRoomIdGenerator {
  secure: () => string;
  friendly: () => string;
  timed: (prefix?: string) => string;
  validate: (roomId: string | undefined | null) => boolean;
  hash: (roomId: string) => string;
}

// Room ID utilities
export const RoomIdGenerator: IRoomIdGenerator = {
